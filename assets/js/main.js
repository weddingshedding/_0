(function(){
  ...
  if(window.gsap){
    ...
    gsap.utils.toArray('.reveal').forEach(el=>{
      gsap.to(el,{opacity:1,y:0,duration:.9,ease:'power2.out',scrollTrigger:{trigger:el,start:'top 86%'}});
    });
    gsap.utils.toArray('.stat strong').forEach(num=>{
      const val=parseInt(num.dataset.count||num.textContent,10)||0;
      gsap.fromTo(num,{innerText:0},{innerText:val,duration:2,snap:{innerText:1},scrollTrigger:{trigger:num,start:'top 88%'}});
    });
  }else{
