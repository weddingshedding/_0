...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'upload_media') {
            ...
            add_media_item($type, trim($_POST['title'] ?? ''), trim($_POST['category'] ?? ''), $file, trim($_POST['alt_text'] ?? ''), (int)($_POST['sort_order'] ?? 0));
            $message = ucfirst($type) . ' uploaded successfully.';
        }
        if ($action === 'edit_media') {
            $id = (int)($_POST['id'] ?? 0);
            update_media_item($id, trim($_POST['title'] ?? ''), trim($_POST['category'] ?? ''), trim($_POST['alt_text'] ?? ''), (int)($_POST['sort_order'] ?? 0));
            $message = 'Gallery item updated.';
        }
        if ($action === 'delete_media') {
            $id = (int)($_POST['id'] ?? 0);
            $filePath = get_media_file_path($id);
            if ($filePath) {
                $path = realpath(__DIR__ . '/../' . $filePath);
                $root = realpath(__DIR__ . '/..');
                if ($path && $root && str_starts_with($path, $root) && is_file($path) && !str_contains($filePath, 'assets/')) {
                    @unlink($path);
                }
            }
...
$messages = get_contact_messages(20);
?>
<!doctype html>
...
<div class="admin-shell">
  ...
  <main class="main">
    ...
    <?php if (!db_ready()): ?><div class="alert">MySQL is not connected yet. Admin uploads and edits are saved in local JSON fallback. For final hosting, import database.sql and update config.php.</div><?php endif; ?>
...
function render_media_admin(array $items, bool $video = false): void
{
    ...
    foreach ($items as $item) {
        ...
        echo '<div class="body">';
        echo '<form method="post" class="inline-edit">';
        echo '<input type="hidden" name="csrf_token" value="' . e(csrf_token()) . '">';
        echo '<input type="hidden" name="action" value="edit_media">';
        echo '<input type="hidden" name="id" value="' . (int)$item['id'] . '">';
        echo '<label>Title</label><input name="title" value="' . e($item['title']) . '">';
        echo '<label>Category</label><input name="category" value="' . e($item['category']) . '">';
        echo '<label>Alt Text</label><input name="alt_text" value="' . e($item['alt_text'] ?? '') . '">';
        echo '<label>Sort Order</label><input type="number" name="sort_order" value="' . (int)($item['sort_order'] ?? 0) . '">';
        echo '<button class="btn gold" type="submit">Save Edit</button>';
        echo '</form>';
        echo '<form method="post" onsubmit="return confirm(\'Delete this media?\')" style="margin-top:10px"><input type="hidden" name="csrf_token" value="' . e(csrf_token()) . '"><input type="hidden" name="action" value="delete_media"><input type="hidden" name="id" value="' . (int)$item['id'] . '"><button class="btn danger" type="submit">Delete</button></form>';
        echo '</div></div>';
    }
    echo '</div>';
}
